...
map<string, string> parameters::param_mkdisk(vector<string> tokens){
    ...
    if (size != -1 && !path.empty()){   
        ...
        if (size <= 0){
            cout << cnst_prmts.RED << "ERROR:" << cnst_prmts.NC << " El valor ingresado en el parametro SIZE debe de ser mayor a 0" << endl;
            map_u.clear();
        }else if (!(fit == "BF" || fit == "FF" || fit == "WF")){
            cout << cnst_prmts.RED << "ERROR:" << cnst_prmts.NC << " El valor ingresado en el parametro FIT es incorrecto" << endl;
            map_u.clear();
        }else if(!(unit == "k" || unit == "m")){
            cout << cnst_prmts.RED << "ERROR:" << cnst_prmts.NC << " El valor ingresado en el parametro UNIT es incorrecto" << endl;
            map_u.clear();
            ...
            cout << cnst_prmts.RED << "ERROR:" << cnst_prmts.NC << " El disco ya existe en la ruta: " << path << endl;
            map_u.clear();
        }
    }else{
        cout << cnst_prmts.RED << "ERROR:" << cnst_prmts.NC << " No ha ingresado algunos de los campos obligatorios (-size, -path)" << endl;
        map_u.clear();
    }
    return map_u;
}
